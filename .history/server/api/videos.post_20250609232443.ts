...
export default defineEventHandler(async (event) => {
  try {
    ...
    // Read request body
    const body = await readBody(event) as VideoSearchBody
    ...
    const handleVideoSearch = await import('~/utils/chains/videoSearchAgent')
    ...
    const videos = await handleVideoSearch.default(
      {
    ...
    console.error('An error occurred while searching videos:', error)
    ...
    throw createError({
      statusCode: 500,
      statusMessage: 'An error occurred while searching videos'
    })
  }
}) 